...
class TLToCustom(val combinational: Boolean = true, val adapterName: Option[String] = None, val stripBits: Int = 0, val wcorrupt: Boolean = true)(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val sourceTable = Wire(Vec(edgeIn.client.endSourceId, out.a.bits.id))
      ...
      val out_arw = Wire(Decoupled(new CustomBundleA(out.params)))
      ...
      out.b.ready := in.d.ready //by dongdeji && !r_wins
      //by dongdeji in.d.valid := Mux(r_wins, out.r.valid, out.b.valid)
      ...
      val d_sel = UIntToOH(out.b.bits.id, edgeOut.master.endId).asBools
